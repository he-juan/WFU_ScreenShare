...
GsRTC.prototype.SIGNAL_EVENT_TYPE = {
    ...
    // (GsPhone发送到Web端的信令类型)
    INVITE_RET: { id: 0x01 << 1, name: 'createMediaSessionRet' },               // 建立会话的回复
    RE_INVITE_RET: { id: 0x01 << 2, name: 'updateMediaSessionRet' },            // 更新会话信息的回复

    // (Web->GsPhone 和 GsPhone->Web 共有信令类型)
    PRESENT: { id: 0x01 << 8, name: 'ctrlPresentation' },                       // 请求开启演示
    PRESENT_RET: { id: 0x01 << 9, name: 'ctrlPresentationRet' },                // 收到请求开启演示的回复信令
    MESSAGE: { id: 0x01 << 4, name: 'sendMessageToUser' },                      // 发送消息
    MESSAGE_RET: { id: 0x01 << 5, name: 'sendMessageToUserRet' },               // 收到消息后的回复信令
    UPDATE_USER_INFO: { id: 0x01 << 6, name: 'updateUserInfo' },                // 更新用户信息
    UPDATE_USER_INFO_RET: { id: 0x01 << 7, name: 'updateUserListRet' },         // 收到更新用户信息后的回复信令
    UPDATE_CANDIDATE_INFO: { id: 0x01 << 10, name: 'updateCandidate' },         // trickle-ice 时用来发送candidate
    UPDATE_CANDIDATE_INFO_RET: { id: 0x01 << 11, name: 'updateCandidateRet' },  // trickle-ice 时收到candidate时的回复信令
    BYE: { id: 0x01 << 3, name: 'destoryMediaSession' },                        // 结束会话
    BYE_RET: { id: 0x01 << 3, name: 'destoryMediaSessionRet' },                 // 结束会话的回复
}
