...
module.exports = function setup(env) {
    ...
    var localLogsDBInterval = setInterval(function () {
        if(env.sessionStorage.dbName){
            ...
            clearInterval(localLogsDBInterval)
        }
    }, 500)
   ...
   // Active `debug` instances.
    ...
    function sessionStorageSave() {
        ...
        if (!env.sessionStorage.dbName || !env.sessionStorage.dbName.match(tabID)) {
            ...
            if(env.sessionStorage.setItem){
                env.sessionStorage.setItem("dbName", "TS_" + tabID);
   ...
   */
        ...
        request.onsuccess = function (e) {
            ...
            store.openCursor().onsuccess = function (event) {
                ...
                if (cursor) {
                    ...
                    callback(localLogs, logStorageFormatTime);
                }
            };
        }
    }
    ...
    function exportLog(day, logLevel, moduleName,dbName) {
        day = parseInt(day) || 0
        moduleName = moduleName || '*'
        logLevel = logLevel || '*'
        createDebug.getLocalLogs(dbName, day, moduleName, logLevel,
           function(logs, time) {
               var fileName = (time || 'meetingLog') + '.txt';
    ...
    removeCodecByPayload: function(session, index, payloads) {
        ...
        payloads.forEach(pt => {
            ...
            if ( typeof(mediaSession.payloads) === 'string' ) {
                mediaSession.payloads = mediaSession.payloads.replace(pt, "").trim();
            }
...
(function (root, factory) {
     ...
     */
    MediaDevice.prototype.enumDevices = function (deviceInfoCallback) {
    ...
    MediaDevice.prototype.updateDeviceInfo = function (deviceInfos, type) {
        ...
        switch (type) {
            case 'cameras':
                ...
                storageInfoList = localStorageDeviceInfo && localStorageDeviceInfo.cameras || []
                break
            case 'microphones':
                deviceInfoList = deviceInfos.microphones
                storageInfoList = localStorageDeviceInfo && localStorageDeviceInfo.microphones || []
                break
            case 'speakers':
                deviceInfoList = deviceInfos.speakers
                storageInfoList = localStorageDeviceInfo && localStorageDeviceInfo.speakers || []
                break
            default:
                break
        }
    ...
    MediaDevice.prototype.getStreamWithExactConstraints = async function () {
        ...
        for (var j = 0; j < mediaDevice.cameras.length; j++) {
            ...
            if (mediaDevice.cameras[j].capability && mediaDevice.cameras[j].capability.length > 0) {
                log.log("this device has already get resolution before: " + mediaDevice.cameras[j].label)
                continue
            }
     ...
     * 判断共享桌面时是否支持共享音频
     * */
    MediaDevice.prototype.isAudioShareSupport = function(){
        let result = false;
        let browserDetail = MediaDevice.getBrowserDetail();
        let ifEdg = navigator.userAgent.indexOf('Edg');
        if((browserDetail.browser === "chrome" &&  ifEdg > 0 && browserDetail.version >= 79)
            ||(browserDetail.browser === "chrome" && ifEdg < 0 && browserDetail.version >= 74)
    ...
    MediaDevice.prototype.getMedia = async function (data, constraints) {
        ...
        if (data.streamType === 'audio' || data.streamType === 'video') {
            ...
            if (data.streamType === 'video' && stream && stream.getVideoTracks().length && stream.active === true) {
                ...
                // audio and video not
                // log.info("getUserMedia constraints: " + JSON.stringify(constraints, null, '  '))
            ...
            constraints.audio = this.isAudioShareSupport()
            if (navigator.getDisplayMedia) {
                ...
                constraints = This.getConstraints({  streamType: 'screenShare', source: 'window'}, true)
                navigator.mediaDevices.getUserMedia(constraints).then(onGetStreamSuccess).catch(onGetStreamFailed)
            }else if(navigator.getUserMedia){
                constraints = This.getConstraints({  streamType: 'screenShare', source: 'window'}, true)
                navigator.getUserMedia(constraints).then(onGetStreamSuccess).catch(onGetStreamFailed)
            }else {
                log.info('getDisplayMedia is not supported by current browser')
                // 使用插件共享桌面
                data.callback(gsRTC.CODE_TYPE.NOT_SUPPORT_SCREEN_SHARE)
            }
        }
    }
    ...
    MediaDevice.prototype.getNextConstraints = function (data) {
        ...
        let settings = {
            frameRate: lastSettings.video.frameRate.exact || lastSettings.video.frameRate.ideal || lastSettings.video.frameRate,
            width: lastSettings.video.width.exact || lastSettings.video.width.ideal || lastSettings.video.width,
            height: lastSettings.video.height.exact || lastSettings.video.height.ideal || lastSettings.video.height,
            deviceId: lastSettings.video.deviceId.exact || lastSettings.video.deviceId.ideal || lastSettings.video.deviceId,
        }
        ...
        return {
            ...
            frameRate: (nextConstraints && nextConstraints.frameRate) || (data.constraints && data.constraints.frameRate) || (settings && settings.frameRate) || 15,
            width: (nextConstraints && nextConstraints.width) || (data.constraints && data.constraints.width) || (settings && settings.width) || 1280,
            height: (nextConstraints && nextConstraints.height) || (data.constraints && data.constraints.height) || (settings && settings.height) || 720,
        }
    }
    ...
    MediaDevice.prototype.getScreenShareConstraints = function (data) {
         ...
         * for Edge
         */
        if (MediaDevice.getBrowserDetail().browser === "edge") {
            if (MediaDevice.getBrowserDetail().version >= 17134 && !!navigator.getDisplayMedia) {
                screenConstraints = {
                    audio: false,
                    video: {
         ...
         */
        if ((MediaDevice.getBrowserDetail().browser === "ie" || MediaDevice.getBrowserDetail().browser === "safari") && MediaDevice.getBrowserDetail().isWebRTCPluginInstalled === true) {
            screenConstraints = {
                audio: false,
                video: {
...
WebSocketInstance.prototype.createWebSocket = function(data){
    ...
    ws.onclose = function (event) {
        ...
        if(gsRTC.sharingPermission !== 4  && gsRTC.sharingPermission !== 5){
            log.warn("同步按钮状态")
